<?php
	...
	function pyd_smugslider_shortcode( $pyd_smug_atts ) {
		global $add_my_script, $pydsmug_pydapi, $pydsmug_api;
		$add_my_script = true;

		$pyd_smug_atts = shortcode_atts( array(
			                                 'albumid'   => '',
			                                 'albumkey'  => '',
			                                 'albumtype' => 'slider',
			                                 'imagesize' => 'MediumURL',
			                                 'imagelink' => 'LightboxURL'
		                                 ), $pyd_smug_atts, 'pydsmugmugslider' );

		$albumid   = $pyd_smug_atts[ 'albumid' ];
		$albumkey  = $pyd_smug_atts[ 'albumkey' ];
		$albumtype = $pyd_smug_atts[ 'albumtype' ];
		$imagesize = $pyd_smug_atts[ 'imagesize' ];
		$imagelink = $pyd_smug_atts[ 'imagelink' ];
		...
		try {
			...
			if ( $images != null ) {
				...
				if ( $pydsmug_get_transient === false || empty( $pydsmug_get_transient ) ) {
